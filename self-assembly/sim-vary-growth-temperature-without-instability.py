...
extra_gl = [
    (f"e{i}", f"w{j}", 1.0)
    for (i, j) in zip([7, 8, 9, 10, 11, 12], [1, 5, 4, 3, 2, 6], strict=True)
] + [
    (f"s{i}", f"n{j}", 1.0) for (i, j) in zip([1, 5, 4, 3, 2], [5, 4, 3, 2, 6])
]  # removed 7 to prevent periodic annoyance

# Error pathway glues
error_gl = [(f"e{i}", f"w{j}", 1) for (i, j) in [(5, 8), (3, 10)]]
...
# Error pathway glues
error_gl = [(f"e{i}", f"w{j}", 1) for (i, j) in [(5, 8), (4, 9), (3, 10), (2, 11)]]
...
# Error pathway glues
error_gl = [(f"e{i}", f"w{j}", 1) for (i, j) in [(5, 8), (4, 9)]]
...
# Error pathway glues
error_gl = [(f"e{i}", f"w{j}", 1) for (i, j) in [(5, 8), (2, 11)]]
...
def sweep_growthtemp(
    ts: rg.TileSet,
    growgse: np.ndarray,
    output_file: tb.File,
    melttime: int = 0,
    meltgse: float | None = None,
    ...
    for growgse_val in growgse:
        res = varsims(
            ts,
            gsemelt=growgse_val if meltgse is None else meltgse,
            melttime = melttime,
            meltperiod = meltperiod,  # trial_period,
            gsegrow=growgse_val,
            timelimit=maxtime,
            smax=targetsize,  # 400,
        ...
        for j in range(nsims):
            trace_table = h5f.create_table(
                trace_group_node,
                f"trace_{growgse_val*100:.0f}_{j}",
                SimTrace,
            ...
            trace_table.append(
                [
                    (
                        res["times_array"][j][k],
                        res["sizes_array"][j][k],
                        res["mismatches_array"][j][k],
                    )
                    for k in range(res["lastcycle"][j])
                ]
            )
            trace_table.flush()

        row["final_tiles_array"] = np.array(
            [state.canvas_view for state in res["states"]]
        )
        row["mismatch_locs_array"] = np.array(
            [res["system"].calc_mismatch_locations(state) for state in res["states"]]
        )
        row.append()
        restable.flush()

        print(
            f"{growgse_val:.3f}\t{meltperiod:.2f}\t{melttime:.2f}\t{res['invtime']:.3e}\t{res['mm']:.3f}",
            flush=True,
        )
    return h5f
...
with tb.open_file(
    ...
    for s in [
        ...
        print(f"Starting {s} melttime sweep")
        sweep_growthtemp(
            systems[s],
            np.arange(8.75, 10.0, 0.05),
            meltgse=None,
            output_file=h5f,
            output_group=f"/{s}",
            output_name="results",
